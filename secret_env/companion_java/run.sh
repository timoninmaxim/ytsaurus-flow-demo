...
# point at the flow_server of the Flow release the jars were built against, taken out of the
# release image:
#   docker create --name flow ghcr.io/ytsaurus/flow-java:<version>
#   docker cp flow:/usr/bin/flow_server ~/flow_server && docker rm flow
# or, without docker, ./fetch_image_file.py ghcr.io/ytsaurus/flow-java:<version> /usr/bin/flow_server ~/flow_server
# from the repo root. A test release is ghcr.io/ytsaurus/flow-java-nightly:dev-<version>.
#
