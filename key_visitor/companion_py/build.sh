#!/usr/bin/env bash
# Builds companion_bundle.tgz -- everything the Python visit tester needs at runtime inside the
...
#   - main.py (the visit tester).
# Same recipe as companion_python/build.sh; the one addition is wheel reuse: the
# ytsaurus-flow-companion wheel is taken from $WHEEL or from companion_python's build dir if one
# is already there, and only built from $YTSAURUS_SRC/yt/yt/flow/tools/companion otherwise.
set -euo pipefail
cd "$(dirname "$0")"
