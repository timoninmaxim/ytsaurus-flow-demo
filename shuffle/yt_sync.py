# Creates the Cypress objects for the shuffle scenario: the pipeline node, the
# input queue (4 tablets) with its consumer, and the single-tablet output queue.
#
...
# The input queue is sharded so the source itself starts out partitioned; every
# shuffle hop then repartitions that stream again.
INPUT_QUEUE_TABLET_COUNT = 4

CLUSTER = os.environ["YT_PROXY"]
FOLDER = os.environ["YT_DEV_ROOT"] + "/shuffle"
...
def main():
    ...
    tables = {
        ...
        for name, tablet_count in (("input_queue", INPUT_QUEUE_TABLET_COUNT), ("output_queue", 1))
    }
