...
# Both steps are conditional: once the operation has exited there is no controller left to serve
# pipeline state at all.
set -euo pipefail

PIPELINE="$YT_DEV_ROOT/swift_map_batching/pipeline"
...
case "$STATE" in
    unreachable)
        ...
        ;;
    stopped)
        echo "pipeline is already stopped"
        ;;
    *)
