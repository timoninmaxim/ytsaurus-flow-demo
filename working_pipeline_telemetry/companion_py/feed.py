...
# fail key), plus, every --fail-every seconds, one row with key = fail_key and a unique
# "data" value: each such row makes the Python reader raise fail_attempts times and then
# pass (see main.py), i.e. one genuine job failure per fail row.
#
...
def main():
    ...
    queue = os.environ["YT_DEV_ROOT"] + "/working_pipeline_telemetry_py/input_queue"
