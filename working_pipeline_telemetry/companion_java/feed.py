...
# fail or error key), plus, every --fail-every seconds, one row with key = fail_key and a
# unique "data" value: each such row makes the Java reader throw a RuntimeException fail_attempts
# times and then pass (see FailingRead.java), i.e. one genuine job failure per fail row. A row
# with key = error_key (1101) — the java.lang.Error-shaped counterpart — is inserted manually once
...
def main():
    ...
    queue = os.environ["YT_DEV_ROOT"] + "/working_pipeline_telemetry_java/input_queue"
