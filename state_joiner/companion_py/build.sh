#!/usr/bin/env bash
# Builds companion_bundle.tgz -- everything the Python accumulator/joiner need at runtime inside
# the worker's vanilla job (whose image has only a bare python3.8 while the companion SDK needs
# 3.9+):
...
#   - main.py (the accumulator and the joiner).
# Same recipe as word_count_sync/companion_py/build.sh: the ytsaurus-flow-companion wheel is taken
