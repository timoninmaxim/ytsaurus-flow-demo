# Feeds the input queue of the Java-companion variant — the stand-in for the C++
...
# backlog line of companion_java/measure.py. One process tops out around 15K rows/s
...
#   python3 companion_java/feed.py --duration 900 &
#   python3 companion_java/feed.py --duration 900 &
...
#   python3 companion_java/feed.py --duration 600
...
def main():
    ...
    queue = os.environ["YT_DEV_ROOT"] + "/transform_high_throughput_java/input_queue"
    ...
    while time.time() < deadline:
        ...
        # The single active tablet node freezes all writes with code 1703 when its memory
        # fills up (the store flusher catches up within seconds); ride the freeze out
        # instead of dying — the achieved-rate report makes the stall visible.
        while True:
            try:
                client.insert_rows(
                    queue,
