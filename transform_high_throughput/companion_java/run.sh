...
# TJavaCompanionManager classpath and execs flow_server. The source is a queue read infinitely,
# so this streams the controller log until Ctrl-C detaches (the pipeline keeps running).
#
...
exec "${JAVA:-java}" -Djava.library.path="$LIBS" -cp "$LIBS/*" \
    tech.ytsaurus.flow.demo.transformhighthroughput.TransformHighThroughputMain \
