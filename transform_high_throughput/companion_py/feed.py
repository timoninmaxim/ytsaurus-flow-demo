# Feeds the input queue of the Python-companion variant — the stand-in for the C++
...
# measured: keep --rate well above the expected throughput and check the backlog
# line of companion_py/measure.py. The per-second report prints the achieved
# cumulative rate; a "behind" warning means the feeder itself cannot keep the pace
# and --rate is effectively lower.
#
# Run after sourcing your env file (see the repo README), alongside measure.py:
#   python3 companion_py/feed.py --duration 600
...
def main():
    ...
    queue = os.environ["YT_DEV_ROOT"] + "/transform_high_throughput_py/input_queue"
