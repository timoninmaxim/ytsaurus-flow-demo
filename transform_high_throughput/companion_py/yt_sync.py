# Creates the Cypress objects for the Python-companion variant of the
# transform_high_throughput scenario, under its own root
# ($YT_DEV_ROOT/transform_high_throughput_py) so the C++ variant's objects stay
# inspectable. Unlike the C++ variant (whose input is the in-binary TRandomSource,
# so it needs nothing but the pipeline node and the output queue), this variant
# reads a queue: the stock flow_server does not link the random connector, so the
# input is a queue fed from the dev host (see feed.py). Two tablets on the input
# queue give the reader the C++ variant's two source partitions.
#
# Run after sourcing your env file (see the repo README):
#   python3 companion_py/yt_sync.py
...
FOLDER = os.environ["YT_DEV_ROOT"] + "/transform_high_throughput_py"
