...
# Requires computation_cycles_companion.stripped built by ./build.sh.
set -euo pipefail
cd "$(dirname "$0")"

FLOW_BIN="${FLOW_BIN:-$HOME/ytsaurus/yt/yt/flow/bin/flow_server/flow_server}"

if [ ! -f computation_cycles_companion.stripped ]; then
    echo "computation_cycles_companion.stripped not found — run ./build.sh first" >&2
    exit 1
fi
