#!/usr/bin/env bash
# Stops one variant's pipeline, then aborts the vanilla operation the runner recorded on the
# pipeline node. Both steps are conditional: `completed` is a final state that refuses
# stop-pipeline, and once the operation has exited there is no controller left to serve pipeline
# state at all.
#
#   ./stop.sh {swift|delete|aggregate}
set -euo pipefail
...
PIPELINE="$YT_DEV_ROOT/sorted_dynamic_table/$VARIANT/pipeline"
